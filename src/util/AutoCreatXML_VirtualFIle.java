    ...
    public String readVirtualFile(VirtualFile[] files, String testngxmlpath)
    {
        ...
        for (int i = 0; i < files.length; i++)
        {
            ...
            Element test = root.addElement("test")
                ...
                .addAttribute("preserve-order", "true").addAttribute("name", testname);
            
            //创建节点classes，无属性
            Element classes = test.addElement("classes");
            List<File> fiList = FileUtil.getFilesWithOutDir_spring(path);
            if (fiList == null)
            {
            ...
            for (File f : fiList)
            {
                ...
                if (f.getName().endsWith(".java"))
                {
                    ...
                    String fileContain = FileUtil.readTxtFileReturn_without_comments(f, "").trim();
            ...
            System.out.println("\n\n#####\n#####\txml文件在：" + outtpath);
            FileUtil.writeStrToFile(document.asXML(), outtpath, false);
